<?php
	...
	class SubmitEventController extends PHPWebTemplate
	{
		public function __invoke(Request $request): Response {
			$this->setPageTitle('Submit Event');
			return $this->render([$this, 'renderContents']);
		}
		
		private function display_options($options, $current)
		{
			...
			<form action="/submit-event.php" method="post">
				<table border="0" cellpadding="3" class="standard">
					<tr>
						...
						<td>
							<select name="smonth"><option></option><?php $this->display_options($months, $_POST['smonth'])?></select>
							<input type="text" name="sday" size="2" maxlength="2" value="<?php echo htmlentities((string)$_POST['sday'], ENT_QUOTES | ENT_IGNORE, 'UTF-8')?>">
							<input type="text" name="syear" size="4" maxlength="4" value="<?php echo $_POST['syear'] ? htmlentities((string)$_POST['syear'], ENT_QUOTES | ENT_IGNORE, 'UTF-8') : date("Y")?>">
					...
					<tr>
						...
						<td>
							<select name="emonth"><option></option><?php $this->display_options($months, $_POST['emonth'])?></select>
							<input type="text" name="eday" size="2" maxlength="2" value="<?php echo htmlentities((string)$_POST['eday'], ENT_QUOTES | ENT_IGNORE, 'UTF-8')?>">
							<input type="text" name="eyear" size="4" maxlength="4" value="<?php echo $_POST['eyear'] ? htmlentities((string)$_POST['eyear'], ENT_QUOTES | ENT_IGNORE, 'UTF-8') : date("Y")?>">
					...
					<tr>
						...
						<td>
							<select name="recur"><option></option><?php $this->display_options($re, $_POST['recur'])?></select>
							<select name="recur_day"><option></option><?php $this->display_options($days, $_POST['recur_day'])?></select>
					...
					<tr>
						...
						<td>
							<select name="country" class="max">
								...
								<?php $this->display_options(Countries::COUNTRIES_BY_ALPHA_3, $_POST['country']);?>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							<select name="category" class="max">
								<?php
									...
									$this->display_options($cat, $_POST['category']);
								?>
							</select>
						</td>
					</tr>
					...
					<tr>
						...
						<td><select name="sane"><?php $this->display_options(array("I, Robot", "I used to be", "WTF?", "No, but I'd still want to submit this", "Yes"), "2"); ?></select></td>
					</tr>
				</table>
			</form>
			<?php
